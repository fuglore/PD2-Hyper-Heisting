...
function CopLogicBase.is_obstructed(data, objective, strictness, attention)
	--bots should no longer get interrupted by objectives, now cops are simply given the option of receiving new objectives rather than being FORCED into another objective which should make them feel less dumb
	local my_data = data.internal_data
	...
	if not my_data.next_allowed_obs_t or my_data.next_allowed_obs_t < data.t then
		
		if not Global.game_settings.one_down then
			if objective.interrupt_suppression and data.is_suppressed then
				if my_data and mid_fight then
