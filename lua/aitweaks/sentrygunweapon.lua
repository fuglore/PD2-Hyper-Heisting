...
function SentryGunWeapon:_fire_raycast(from_pos, direction, shoot_player, target_unit)
	...
	local shouldnt_suppress = nil
	
	if (not col_ray or col_ray.unit ~= target_unit) and target_unit and target_unit:character_damage() and target_unit:character_damage().build_suppression and not tweak_data.weapon[self._name_id].dontsuppressonmiss then
		target_unit:character_damage():build_suppression(self._suppression)
		shouldnt_suppress = true
	end
	...
	if shoot_player then
		...
		if player_hit then
			...
			if target_unit and target_unit:character_damage() and target_unit:character_damage().build_suppression and not shouldnt_suppress then
				target_unit:character_damage():build_suppression(self._suppression)
			end
		end
	end
