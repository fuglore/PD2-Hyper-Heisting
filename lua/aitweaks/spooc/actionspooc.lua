...
function ActionSpooc:_chk_can_strike(kick_after_landing)
...
function ActionSpooc:anim_act_clbk(anim_act)
	...
	if flying_strike and not kick_after_landing then
		...
		local target_dis_xy = target_vec:length()
		...
		local fuckingpos = mvec3_cpy(self._target_unit:movement():m_pos())
		
		fuckingpos = fuckingpos - my_pos
...
function ActionSpooc.chk_can_start_spooc_sprint(unit, target_unit)
	...
	if is_client then
		slot_mask = managers.slot:get_mask("world_geometry", "vehicles", "enemy_shield_check")
		...
		if gnd_z < new_pos.z then
			...
			if self._is_local then
				...
				if target_unit and self:_chk_invalid_beating_unit_status(target_unit) then
					...
					if self:_chk_can_strike(true) then
						local detect_stop_sound = self:get_sound_event("detect_stop")
...
function ActionSpooc:_get_current_max_walk_speed(move_dir)
	...
	if not is_hostage then
		speed_modifier = speed_modifier + managers.groupai:state()._enemy_speed_mul or 1
