...
function ActionSpooc.chk_can_start_spooc_sprint(unit, target_unit)
	...
	if is_client then
		client_my_head_pos = unit:movement():m_head_pos()
		client_target_head_pos = target_unit:movement():m_head_pos()

		local target_dis = mvec3_dis(client_my_head_pos, client_target_head_pos)
	...
	if is_client then
		local extra_ray = unit:raycast("ray", client_my_head_pos, client_target_head_pos, "slot_mask", managers.slot:get_mask("world_geometry", "vehicles", "enemy_shield_check"), "ignore_unit", target_unit, "report")
