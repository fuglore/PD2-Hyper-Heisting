...
function GroupAIStateBase:criminal_hurt_drama_armor(unit, attacker)
	...
	if alive_g(attacker) then
		local drama_amount = 0.1
		local max_dis = drama_data.max_dis_armor
		local dis_lerp = math_min(1, mvec3_dis(attacker:movement():m_pos(), unit:movement():m_pos()) / drama_data.max_dis_armor)
		dis_lerp = math_lerp(1, drama_data.max_dis_mul_armor, dis_lerp)
...
function GroupAIStateBase:_execute_so(so_data, so_rooms, so_administered)
	...
	if ai_group == "enemies" then
		for e_key, enemy_unit_data in pairs_g(self._police) do
	...
	elseif ai_group == "friendlies" then
		for u_key, u_unit_data in pairs_g(self._ai_criminals) do
	...
	elseif ai_group == "civilians" then
		for u_key, u_unit_data in pairs_g(managers.enemy:all_civilians()) do
			if check_allowed(u_key, u_unit_data) then
				local dis = mvec3_dis_sq(u_unit_data.m_pos, pos)
		...
		for u_key, civ_unit_data in pairs_g(managers.enemy:all_civilians()) do
